...
bool ShaderProgram::loadShaders(const QString & vsFilename, const QString & fsFilename)
{
    ...
    glCompileShader(vs);
    if (!checkCompilationResult(vs)) return false;

    glCompileShader(fs);
    if (!checkCompilationResult(fs)) return false;
...
bool ShaderProgram::checkCompilationResult(GLuint shader)
{
    int status = 0;
    ...
    if (status == GL_FALSE)
    {
        ...
        qWarning() << "Shader program : failed to compile. " << errorLog;
        return false;
    }
    return true;
}
